...
func (s *Session) RegisterPriceTable(payment PaymentMethod, height uint64) (rhp3.HostPriceTable, error) {
	...
	} else if err := s.processPayment(stream, payment, pt.UpdatePriceTableCost, height); err != nil {
...
func (s *Session) FundAccount(account rhp3.Account, payment PaymentMethod, amount types.Currency, height uint64) (types.Currency, error) {
	stream := s.t.DialStream()
	defer stream.Close()
	...
	} else if err := s.processPayment(stream, payment, s.pt.FundAccountCost.Add(amount), height); err != nil {
...
func (s *Session) AccountBalance(account rhp3.Account, payment PaymentMethod, height uint64) (types.Currency, error) {
	stream := s.t.DialStream()
	defer stream.Close()

	if err := stream.WriteRequest(rhp3.RPCAccountBalanceID, &s.pt.UID); err != nil {
		return types.ZeroCurrency, fmt.Errorf("failed to write request: %w", err)
	} else if err := s.processPayment(stream, payment, s.pt.AccountBalanceCost, height); err != nil {
		return types.ZeroCurrency, fmt.Errorf("failed to pay: %w", err)
	}
...
func (s *Session) StoreSector(sector *[rhp2.SectorSize]byte, duration uint64, payment PaymentMethod, budget types.Currency, height uint64) error {
	stream := s.t.DialStream()
	defer stream.Close()
	...
	} else if s.processPayment(stream, payment, s.pt.InitBaseCost.Add(budget), height) != nil {
...
func (s *Session) AppendSector(sector *[rhp2.SectorSize]byte, revision *rhp2.ContractRevision, renterKey types.PrivateKey, payment PaymentMethod, budget types.Currency, height uint64) (types.Currency, error) {
	stream := s.t.DialStream()
	defer stream.Close()
	...
	if err := stream.WriteRequest(rhp3.RPCExecuteProgramID, &s.pt.UID); err != nil {
		return types.ZeroCurrency, fmt.Errorf("failed to write request: %w", err)
	} else if err := s.processPayment(stream, payment, s.pt.InitBaseCost.Add(budget), height); err != nil {
		return types.ZeroCurrency, fmt.Errorf("failed to pay: %w", err)
	} else if err := stream.WriteResponse(&req); err != nil {
		return types.ZeroCurrency, fmt.Errorf("failed to write response: %w", err)
	}
	...
	if err := stream.ReadResponse(&resp, 4096); err != nil {
		...
		return types.ZeroCurrency, fmt.Errorf("failed to execute instruction: %w", resp.Error)
	...
	if revised.MissedHostPayout().Cmp(transfer) < 0 {
		transfer = revised.MissedHostPayout()
	}
...
func (s *Session) ReadSector(root types.Hash256, offset, length uint64, payment PaymentMethod, budget types.Currency, height uint64) ([]byte, types.Currency, error) {
	stream := s.t.DialStream()
	defer stream.Close()
