...
// ScanSettings scans the host's settings and returns them.
func ScanSettings(ctx context.Context, hostKey types.PublicKey, hostAddr string) (settings rhp2.HostSettings, err error) {
	s, err := NewSession(ctx, hostKey, hostAddr, nil, nil, nil)
	if err != nil {
