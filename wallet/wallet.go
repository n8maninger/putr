...
type (
	...
	// address and uses the Sia Central API to track balance and send
	// transactions.
	SingleAddressLiteWallet struct {
	...
	// A SiacoinElement pairs a SiacoinOutput with its ID
...
func (sw *SingleAddressLiteWallet) Address() types.UnlockHash {
	return sw.addr
}

// Balance returns the wallet's balance.
func (sw *SingleAddressLiteWallet) Balance() (balance types.Currency, _ error) {
...
func (sw *SingleAddressLiteWallet) SpendableUTXOs() (spendable []SiacoinElement, _ error) {
	sw.mu.Lock()
	defer sw.mu.Unlock()
...
func (sw *SingleAddressLiteWallet) FundTransaction(txn *types.Transaction, amount types.Currency) ([]crypto.Hash, func(), error) {
	if amount.IsZero() {
		return nil, nil, nil
	}
...
func (sw *SingleAddressLiteWallet) Redistribute(outputs uint64, amount types.Currency) (types.Transaction, func(), error) {
	// prepare all outputs
...
// NewSeedPhrase returns a new seed phrase.
func NewSeedPhrase() string {
	return wallet.NewSeedPhrase()
}

// New initializes a new SingleAddressLiteWallet.
func New(recoveryPhrase string) (*SingleAddressLiteWallet, error) {
	key, err := wallet.KeyFromPhrase(recoveryPhrase)
	if err != nil {
		return nil, fmt.Errorf("failed to create seed: %w", err)
	}
	w := &SingleAddressLiteWallet{
